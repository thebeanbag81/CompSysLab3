...
// Ethan Hart, Nicholas Dirschel, Jacob Crouss
...
enum opcode {
    add, addi, sub, mul, beq, lw, sw, haltSimulation
};
...
void progScanner(char *c) {
    ...
    if (token[0] == "") {
        printf("Illegal Opcode detected");
        exit(0);
    }
    ...
    if ((strcmp(token[0], "lw") == 0) || (strcmp(token[0], "sw") == 0)) {
        ...
        if (leftBracket != rightBracket) {
            printf("Mismatched or Missing Parenthesis");
            exit(0);
        }
        else {
            int k = 0;
...
void ID() {
    ...
    if ((IFID.instruction.opcode == add) || (IFID.instruction.opcode == sub) ||
        (IFID.instruction.opcode == mul)) { //add, sub, and mult opcodes will follow this statement
...
void WB() {
    ...
    switch (MEMWB.instruction.opcode) {
        ...
        case beq:
        case addi:
            mips_reg[MEMWB.instruction.rt] = MEMWB.data;
            WB_counter++;
            break;
        case add:
...
main(int argc, char *argv[]) {
    ...
    ifUtil = IF_counter / (double) sim_cycle;
    idUtil = ID_counter / (double) sim_cycle;
    exUtil = EX_counter / (double) sim_cycle;
    memUtil = MEM_counter / (double) sim_cycle;
    wbUtil = WB_counter / (double) sim_cycle;
